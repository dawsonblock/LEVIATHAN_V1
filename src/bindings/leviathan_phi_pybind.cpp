...
// Pybind11 bindings for the GPU-accelerated Φ solver
...
// Forward declaration for accessing d_theta from LeviathanHandle
extern "C" {
...
public:
  ...
  void accumulate_from_host(py::array_t<float> theta) {
    // Upload theta to device temporarily, then accumulate
...
PYBIND11_MODULE(leviathan_phi, m) {
  m.doc() = "Leviathan GPU-Accelerated IIT Φ Solver";

  py::class_<GPUPhiWorker>(m, "GPUPhiWorker")
      .def(py::init<py::array_t<int>>(), py::arg("hub_indices"),
           "Initialize GPU Φ worker with hub node indices")
      .def("accumulate", &GPUPhiWorker::accumulate_from_host, py::arg("theta"),
           "Accumulate TPM transition from phase snapshot (host array)")
      .def("compute", &GPUPhiWorker::compute,
           "Compute Φ from accumulated TPM (returns float)")
      .def("reset", &GPUPhiWorker::reset,
           "Reset TPM transition counts for new window")
      .def("get_transition_count", &GPUPhiWorker::get_transition_count,
           "Get number of accumulated transitions");
}
